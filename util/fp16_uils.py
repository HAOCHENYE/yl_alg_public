        ...
        def new_func(*args, **kwargs):
            ...
            if not isinstance(args[0], torch.nn.Module):
                raise TypeError('@auto_fp16 can only be used to decorate the '
                                'method of nn.Module')
            fp16_enabled = hasattr(args[0], 'fp16_enabled') and args[0].fp16_enabled
            if not fp16_enabled:
                target_type = torch.float
            else:
                target_type = torch.half
            ...
            if args:
                ...
                for i, arg_name in enumerate(arg_names):
                    if arg_name in args_to_cast:
                        new_args.append(
                            cast_tensor_type(args[i], torch.float, target_type))
                    else:
                ...
                for arg_name, arg_value in kwargs.items():
                    if arg_name in args_to_cast:
                        new_kwargs[arg_name] = cast_tensor_type(
                            arg_value, torch.float, target_type)
                    else:
                        new_kwargs[arg_name] = arg_value
            # apply converted arguments to the decorated method
            if (TORCH_VERSION != 'parrots' and
                    digit_version(TORCH_VERSION) >= digit_version('1.6.0')) and fp16_enabled:
