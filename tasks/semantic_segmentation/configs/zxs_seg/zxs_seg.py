...
model = dict(
    type='ZxsSeg',
    backbone=dict(
        type='SandNetDBBAPNVideoSmallNarrowMod'),
    neck=None,
    seg_head=dict(
        type='ZxsHead'
        ))
...
train_pipline = [
            ...
            dict(type="ResizeImage",
                 img_scale=(384, 384),
                 pad_val=0,
                 resize_keys=['img', 'gt_mask', 'prior_mask', 'ori_img']
                 ),
            ...
            dict(type='Formatting',
                 collect_key=["gt_mask", "prior_mask", "img", 'ori_img']),
        ]
...
log_level = 'INFO'
work_dir = './tasks/semantic_segmentation/work_dirs/zxs_seg_dist'
load_from = None
resume_from = None
# resume_from = None
workflow = [('train', 1)]
