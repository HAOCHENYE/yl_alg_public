...
def pytorch2onnx(model,
    ...
    img = torch.zeros((1, 3, 224, 384))
    mask = torch.zeros((1, 1, 224, 384))
    torch.onnx.export(
        model,
        [img, mask],
        output_file,
...
def parse_args():
    ...
    parser.add_argument('--output-names', type=str, nargs='+', default=["pred_map"],help='output names of network')
    parser.add_argument('--input-names', type=str, nargs='+', default=["image", "mask"], help='input names of network')
    parser.add_argument(
        '--shape',
        type=int,
        nargs='+',
