...
class WiderFaceEvaluator(object):
    def __init__(self, dataset, gt_path, **kwargs):
    ...
    def _parse_results(self, results):
        ...
        for i, result in enumerate(results):
            ...
            batch_size = len(img_metas['filename'])
            batch_inds = img_metas['batch_inds']
            for batch_ind in range(batch_size):
                file_path = img_metas['filename'][batch_ind]
                path_compose = file_path.split('/')
                event = path_compose[-2]
                img_path = path_compose[-1]
    ...
    def _evaluation(self, pred, logger, iou_thresh=0.5):
        ...
        for setting_id in range(3):
            ...
            for i in pbar:
                ...
                for j in range(len(img_list)):
                    ...
                    pred_recall, proposal_list = self._image_eval(pred_info, gt_boxes, ignore, iou_thresh)
        ...
        print_log("==================== Results ====================", logger=logger)
        print_log("Easy   Val AP: {}".format(aps[0]), logger=logger)
        print_log("Medium Val AP: {}".format(aps[1]), logger=logger)
        print_log("Hard   Val AP: {}".format(aps[2]), logger=logger)
        print_log("=================================================", logger=logger)
        return dict(Easy=aps[0], Medium=aps[1], Hard=aps[2])

    def _get_preds(self, pred_dir):
        ...
        for event in pbar:
            ...
            for imgtxt in event_images:
                imgname, _boxes = self._read_pred_file(os.path.join(event_dir, imgtxt))
    ...
    def _image_eval(pred, gt, ignore, iou_thresh):
        ...
        overlaps = bbox_overlaps(_pred[:, :4].astype(np.float64), _gt)
